...
<body style="margin:0;padding:0;background-color:#fff7ed;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#0f7b6e;">
<div style="display:none;font-size:1px;color:#fff7ed;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;">${escapeHtml(preheader)}</div>
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color:#fff7ed;padding:32px 16px;">
  <tr><td align="center">
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="560" style="background:#ffffff;border-radius:16px;overflow:hidden;max-width:560px;width:100%;box-shadow:0 1px 2px rgba(15,123,110,0.06);">
      <tr><td style="background:#dc4080;padding:36px 32px 28px;text-align:center;">
        <h1 style="margin:0;color:#ffffff;font-size:32px;font-weight:300;letter-spacing:1.5px;line-height:1;">Pink House</h1>
      ...
      <tr><td style="padding:36px 36px 0;">
        <h2 style="margin:0;font-family:'Cormorant Garamond',Georgia,serif;font-weight:500;font-size:28px;color:#0f7b6e;line-height:1.2;">${greeting}</h2>
        <p style="margin:16px 0 0;font-size:15px;line-height:24px;color:#4a8a82;">${escapeHtml(copy.intro)}</p>
      </td></tr>
      <tr><td style="padding:32px 36px 0;">
        ${sectionLabel(copy.stayTitle)}
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="border:1px solid #fce0eb;border-radius:12px;border-collapse:separate;">
      ...
      <tr><td style="padding:24px 36px 0;">
        ...
        <p style="margin:12px 0 0;font-size:12px;line-height:18px;color:#4a8a82;">${escapeHtml(copy.balanceNote)}</p>
        <p style="margin:8px 0 0;font-size:12px;line-height:18px;color:#a8285c;">${escapeHtml(copy.nonRefundableNote)}</p>
      </td></tr>
      <tr><td style="padding:32px 36px 0;">
        ${sectionLabel(copy.contactTitle)}
        <p style="margin:0;font-size:14px;line-height:22px;color:#4a8a82;">${escapeHtml(contactBody)}</p>
      </td></tr>
      <tr><td style="padding:32px 36px 36px;">
        <p style="margin:0;font-family:'Cormorant Garamond',Georgia,serif;font-size:22px;color:#0f7b6e;">${escapeHtml(copy.signOff)}</p>
        <p style="margin:6px 0 0;font-size:14px;color:#0f7b6e;font-weight:600;">${escapeHtml(copy.houseName)}</p>
        <p style="margin:6px 0 0;font-size:12px;color:#4a8a82;">${escapeHtml(copy.houseAddress)}</p>
      </td></tr>
    </table>
  </td></tr>
</table>
...
function sectionLabel(label: string): string {
  return `<p style="margin:0 0 12px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#a8285c;font-weight:600;">${escapeHtml(label)}</p>`;
}

function row(label: string, value: string, isLast: boolean = false): string {
  const border = isLast ? "" : "border-bottom:1px solid #fdf3f7;";
  return `<tr><td style="padding:14px 18px;${border}font-size:13px;color:#4a8a82;">${escapeHtml(label)}</td><td style="padding:14px 18px;${border}font-size:14px;color:#0f7b6e;text-align:right;font-weight:600;">${value}</td></tr>`;
}
...
function rowTotal(label: string, value: string): string {
  return `<tr><td style="padding:14px 18px;font-size:13px;color:#4a8a82;">${escapeHtml(label)}</td><td style="padding:14px 18px;font-size:14px;color:#0f7b6e;text-align:right;font-weight:600;">${value}</td></tr>`;
}
