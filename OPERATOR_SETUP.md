# Operator Setup — Pink House Template
...
3. **`memory/MEMORY.md`** — auto-memory index (e.g. "Pink House deploys to Pages, not Workers").
...
```powershell
Invoke-WebRequest -Uri "https://pinkhousekohsamui.com/api/health" -UseBasicParsing
# expect: {"ok":true}
```
...
```powershell
Invoke-WebRequest -Uri "https://pinkhouse.pages.dev/content" -UseBasicParsing -MaximumRedirection 0
...
node node_modules\wrangler\bin\wrangler.js pages deployment list --project-name pinkhouse
```
...
| Database content   | Cloudflare D1 (`pinkhouse` database)                  | Yes — single source     |
| Uploaded images    | Cloudflare R2 (`pinkhouse-media` bucket)              | Yes — single source     |
...
$env:CLOUDFLARE_ACCOUNT_ID="a28e85c8d0762c4391b0f1dde60b6e75"
node node_modules\wrangler\bin\wrangler.js d1 migrations apply pinkhouse --remote
...
# Deploy
$env:CLOUDFLARE_ACCOUNT_ID="a28e85c8d0762c4391b0f1dde60b6e75"
node node_modules\wrangler\bin\wrangler.js pages deploy .open-next --project-name pinkhouse --commit-dirty=true
...
curl -H "Api-Key: <SMOOBU_API_KEY>" "https://login.smoobu.com/api/rates?apartments[]=3040751&start_date=2026-06-10&end_date=2026-06-12"

# Tail R2 bucket contents
node node_modules\wrangler\bin\wrangler.js r2 object list pinkhouse-media

# Show D1 row count for bookings (sanity check after a booking flow test)
node node_modules\wrangler\bin\wrangler.js d1 execute pinkhouse --remote --command "SELECT COUNT(*) FROM bookings"
```
...
| Service                  | URL                                                                            |
| ------------------------ | ------------------------------------------------------------------------------ |
| Production site          | <https://pinkhousekohsamui.com>                                                |
| Pages dashboard          | <https://dash.cloudflare.com/?to=/:account/pages/view/pinkhouse>               |
| D1 dashboard             | <https://dash.cloudflare.com/?to=/:account/workers/d1>                         |
| R2 dashboard             | <https://dash.cloudflare.com/?to=/:account/r2/default/buckets/pinkhouse-media> |
...
1. Add their email to the **`Editors` policy** in the CF Access "Pink House CMS" app
   (Zero Trust → Access → Applications → Pink House CMS → Edit → Policies).
...
- **D1:** no automatic backup. Schedule a weekly export:
  ```powershell
  $env:CLOUDFLARE_ACCOUNT_ID="a28e85c8d0762c4391b0f1dde60b6e75"
  node node_modules\wrangler\bin\wrangler.js d1 export pinkhouse --remote --output=backups/pinkhouse-$(Get-Date -Format yyyy-MM-dd).sql
  ```
